...
xdata=[]
ydata=[]
raw = serial.Serial("COM17",115200) #start the pyserial object to get raw bits
...
def write_file(name,mode):
    ...
    with open(name,mode)as f:
        ser_data = raw.readline().decode('utf-8')
        if(ser_data):
            f.write(ser_data)
...
#     plt.xticks(rotation=90, ha='right') #making sure everything is properly spaces
